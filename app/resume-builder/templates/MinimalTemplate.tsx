  ...
  return (
    <div className="font-['Georgia',serif] text-[11px] leading-[1.5] text-[#333] p-8 bg-white min-h-[297mm] w-full">
      ...
      <header className="text-center border-b-2 border-[#333] pb-4 mb-5">
        ...
        {personal.jobTitle && (
          <p className="text-sm text-[#555] mb-2">{personal.jobTitle}</p>
        )}
            ...
            {experience.map((exp) => (
              <div key={exp.id}>
                <div className="flex justify-between items-baseline">
                  <h3 className="font-bold text-[#111]">{exp.position}</h3>
      ...
      {skills.length > 0 && (
        <section className="mb-5">
          ...
          <p className="text-[#444]">{skills.join(' • ')}</p>
        </section>
      )}
            ...
            {projects.map((proj) => (
              <div key={proj.id}>
                <div className="flex justify-between items-baseline">
                  ...
                  {proj.link && <span className="text-[10px] text-[#666]">{proj.link}</span>}
                </div>
                {proj.technologies && (
          ...
          <div className="space-y-1">
            {certifications.map((cert) => (
              <div key={cert.id} className="flex justify-between">
                ...
                {cert.date && <span className="text-[10px] text-[#666]">{cert.date}</span>}
              </div>
            ))}
          </div>
        </section>
